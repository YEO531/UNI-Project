...
private:
    ...
    static const int TABLE_SIZE = 100;  // Can be adjusted based on expected number of records
    borrowNode* hashTable;
    bookRecord* bookSystem;
    int currentSize;
    ...
    string getBookName(int bookId, bool& found)
    {
        ...
        if (found)
        {
            // Update all borrowing records with this book ID
...
public:
    borrowingRecord(bookRecord* books)
    {
    ...
    void addRecord()
    {
        ...
        while (true)
        {
            ...
            if (validInput && !input.empty())
            {
                try
                {
                    hashTable[index].student_id = stoll(input); // Using stoll for long long
    ...
    void displayAllRecord()
    {
        ...
        if(choice != 4)
        {
            ...
            bool isDescending = (orderChoice == 2);
        ...
        for (int i = 0; i < currentSize; i++) {
            ...
            cout << "\t\t\tBorrowed Due Date: " << recordsArray[i]->borrowed_date << endl;
            cout << "\t\t\tReturn Date: " << recordsArray[i]->return_date << endl;
    ...
    void loadData()
    {
        ...
        if (!inFile)
        {
            //cout << "\t\t\tNo previous borrowing records found.\n";
            return;
        }
        ...
        inFile.close();
        refreshBookDetails();
        //cout << "\t\t\tBorrowing records loaded successfully!\n";
    }
};
